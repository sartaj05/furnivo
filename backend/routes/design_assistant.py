...
def fallback_brief(payload, products):
    ...
    return {'title': f'{style.title()} {room.title()} concept', 'layout': [f'Anchor the room with a primary seating zone.', f'Keep a clear circulation path around the {room}.', f'Layer {color} accents through textiles, lighting, and artwork.'], 'moodboard': [{'label': 'Palette', 'value': color}, {'label': 'Style', 'value': style}, {'label': 'Material', 'value': payload.get('material', 'Natural wood')}, {'label': 'Lighting', 'value': 'Warm layered light'}], 'design_inputs': {'room': room, 'style': style, 'material': payload.get('material', 'Natural wood'), 'color': color, 'budget': budget, 'image_url': payload.get('image_url', '')}, 'confidence': 'demo', 'next_steps': ['Review the recommended products.', 'Save the selected configuration to a quotation.', 'Confirm dimensions and finish with the designer.'], 'recommendations': [{'product': item.to_dict(), 'reason': f'Fits the {style} direction and requested budget.' if not budget or float(item.price or 0) <= budget else 'Alternative to review above the current budget.'} for item in ranked], 'provider': 'demo-fallback'}
