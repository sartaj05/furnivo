...
export async function getDashboardMetrics() {
  try { return await backendRequest('/dashboard/metrics') }
  catch (error) {
    if (error.status) throw error
    await delay()
    const db = getLocalDb(); const user = JSON.parse(localStorage.getItem('furnivo-user') || 'null'); const role = user?.role || 'admin'; const assigned = demoAssignedOrderIds(db); const orders = assigned ? db.orders.filter((item) => assigned.has(Number(item.id))) : db.orders; const quotes = role === 'client' ? db.quotes : filterDemoAssignedQuotes(db.quotes, db); const leads = role === 'sales' ? db.leads.filter((item) => Number(item.owner_id || user?.id) === Number(user?.id)) : db.leads; const openQuotes = quotes.filter((item) => !['Approved', 'Rejected', 'Expired'].includes(item.status)); const money = (value) => `₹${Number(value || 0).toLocaleString('en-IN', { maximumFractionDigits: 0 })}`
    let cards; let focus
    if (role === 'sales') { const pipeline = openQuotes.reduce((sum, item) => sum + Number(item.amount || 0), 0); const followups = leads.reduce((sum, item) => sum + (item.tasks || []).filter((task) => !task.is_done).length, 0); cards = [{ key: 'quotes', label: 'Open quotations', value: openQuotes.length, display: String(openQuotes.length), helper: 'Assigned or created by you' }, { key: 'pipeline', label: 'Pipeline value', value: pipeline, display: money(pipeline), helper: 'Open quotation value' }, { key: 'leads', label: 'Active leads', value: leads.filter((item) => !['Won', 'Lost'].includes(item.stage)).length, display: String(leads.filter((item) => !['Won', 'Lost'].includes(item.stage)).length), helper: 'Owned opportunities' }, { key: 'followups', label: 'Open follow-ups', value: followups, display: String(followups), helper: 'Tasks needing attention' }]; focus = `${followups} follow-up task(s) need attention.` }
    else if (role === 'designer') { const jobs = filterDemoAssigned(db.productionJobs || [], db); const quality = jobs.filter((item) => item.status === 'Quality check').length; cards = [{ key: 'projects', label: 'Assigned projects', value: orders.length, display: String(orders.length), helper: 'Projects assigned to you' }, { key: 'production', label: 'Active production', value: jobs.filter((item) => !['Complete', 'On hold'].includes(item.status)).length, display: String(jobs.filter((item) => !['Complete', 'On hold'].includes(item.status)).length), helper: 'Jobs in execution' }, { key: 'quality', label: 'Quality checks', value: quality, display: String(quality), helper: 'Jobs awaiting inspection' }, { key: 'products', label: 'Catalog products', value: db.products.length, display: String(db.products.length), helper: 'Reference library' }]; focus = `${quality} production job(s) are waiting for quality review.` }
    else if (role === 'workshop_operator') { const jobs = filterDemoAssigned(db.productionJobs || [], db); const tasks = jobs.flatMap((job) => job.tasks || []); const open = tasks.filter((item) => !['Complete', 'Cancelled'].includes(item.status)).length; const blocked = tasks.filter((item) => item.status === 'Blocked').length; cards = [{ key: 'tasks', label: 'Assigned tasks', value: open, display: String(open), helper: 'Tasks in your workshop queue' }, { key: 'blocked', label: 'Blocked tasks', value: blocked, display: String(blocked), helper: 'Tasks needing help' }, { key: 'projects', label: 'Assigned projects', value: orders.length, display: String(orders.length), helper: 'Projects assigned to you' }, { key: 'products', label: 'Materials reference', value: db.products.length, display: String(db.products.length), helper: 'Catalog reference only' }]; focus = blocked ? `${blocked} task(s) are blocked and need attention.` : 'Keep production tasks moving and hand completed work to quality control.' }
    else if (role === 'client') { cards = [{ key: 'projects', label: 'Active projects', value: orders.filter((item) => !['Complete', 'Cancelled'].includes(item.status)).length, display: String(orders.filter((item) => !['Complete', 'Cancelled'].includes(item.status)).length), helper: 'Your current projects' }, { key: 'quotes', label: 'My quotations', value: quotes.length, display: String(quotes.length), helper: 'Quotes shared with you' }, { key: 'products', label: 'Catalog products', value: db.products.length, display: String(db.products.length), helper: 'Browse the furniture catalog' }]; focus = 'Review your latest quotation or project update.' }
    else { const pipeline = openQuotes.reduce((sum, item) => sum + Number(item.amount || 0), 0); cards = [{ key: 'products', label: 'Catalog products', value: db.products.length, display: String(db.products.length), helper: 'Active catalog products' }, { key: 'quotes', label: 'Open quotations', value: openQuotes.length, display: String(openQuotes.length), helper: 'Current quotation pipeline' }, { key: 'leads', label: 'Active leads', value: db.leads.filter((item) => !['Won', 'Lost'].includes(item.stage)).length, display: String(db.leads.length), helper: 'Open commercial opportunities' }, { key: 'revenue', label: 'Collected revenue', value: 0, display: money(0), helper: `₹${pipeline.toLocaleString('en-IN')} remains in quotation pipeline` }]; focus = `₹${pipeline.toLocaleString('en-IN')} remains in the open quotation pipeline.` }
    return { cards, focus, role, mode: 'demo' }
  }
}
...
export async function getProjectPortal() {
  try { return await backendRequest('/portal') }
    ...
    return { projects, documents: db.contracts.map((contract) => ({ type: 'Contract', number: contract.contract_number, status: contract.status, id: contract.id })).concat(db.invoices.map((invoice) => ({ type: 'Invoice', number: invoice.invoice_number, status: invoice.status, amount: invoice.total, balance: Number(invoice.balance || 0), id: invoice.id, can_pay: Number(invoice.balance || 0) > 0 }))), support_tickets: db.supportTickets, mode: 'demo' }
  }
}
