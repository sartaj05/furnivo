...
export default function InvoicesPage() {
  ...
  async function checkout(invoice) { try { const result = await createCheckout(invoice.id, `checkout-${invoice.id}-${Date.now()}`); window.open(result.item.checkout_url, '_blank') } catch (err) { setError(err.message) } }
