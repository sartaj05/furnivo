...
function LegacySchedulesPage({ canManage, showForm, setShowForm }) {
  const [items, setItems] = useState([]); const [orders, setOrders] = useState([]); const [form, setForm] = useState({ order_id: '', schedule_type: 'Delivery', scheduled_date: '', time_slot: 'Morning', assigned_team: '', proof_url: '', notes: '' }); const [error, setError] = useState('')
...
export default function SchedulesPage() {
  ...
  return <AppShell title="Delivery & installation" eyebrow="Field scheduling" actions={canManage && <button className="button button-small" onClick={() => setShowForm((value) => !value)}>{showForm ? 'Close' : 'Schedule visit'}</button>}>
    <LegacySchedulesPage canManage={canManage} showForm={showForm} setShowForm={setShowForm} />
